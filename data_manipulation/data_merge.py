import json

with open("../data/ram_data_TEST.json", "r", encoding='utf-8') as f:
    ram_data_TEST = json.load(f)

with open("../data/ram_data_TEST2.json", "r", encoding='utf-8') as f:
    ram_data_TEST2 = json.load(f)

with open("../data/ram_data_TEST3.json", "r", encoding='utf-8') as f:
    ram_data_TEST3 = json.load(f)

all_rows = []

all_rows.extend(ram_data_TEST["data"])
all_rows.extend(ram_data_TEST2["data"])
all_rows.extend(ram_data_TEST3["data"])

final_data = {"data": all_rows}

print(len(final_data["data"]))

with open("../data/ram_data.json", "w", encoding='utf-8') as f:
    json.dump(final_data, f, indent=4, ensure_ascii=False)