...
3. **Datová vrstva (Models):** Složky `models/` a `columns/` – obsahují serializované modely, jejich sloupce a scaler (soubory .pkl).
...
3. **Rozhodovací proces:** - Jsou vyplněny CL a Voltage? 
   - ANO -> Pokračuj. 
   - NE -> Doplň defaulty z `default_values.py`, a vytvoří 2 scénáře (Gaming/Office) podle různých chybějících parametrů (Latence a napětí).
4. **Výběr modelu:** Program určí velikost RAM a vybere odpovídající model (PC (malé a větší kapacity) / Servery (do 896GB) / Extrémní kapacity (896+GB)).
5. **Predikce:** Jeden model určí, zda je ram herní, a druhý vypočítá cenu.
...
- **matplotlib:** vizualizace pro účely vývoje.
...
- **Data:** Trénovací data pocházejí z rainforest api a slouží výhradně pro edukační účely.
...
| `IncompatibilityError` | Spuštění na nekompatibilní verzi OS. | Aplikace je určena pro Windows 10 a novější. |
...
- **v2.0:** Nejnovější verze.
- **Známé chyby:**
    - Největší frekvence, na kterých byly modely trénované, mohla být do 10000, ale skutečná data obsahovala maximálně snad jen 6400MHz. Větší frekvence tedy bere stejně jako 6400MHz.
    - Aplikace nepodporuje historické typy RAM (DDR1, SDRAM).
    - Ačkoliv to nevyplývá nijak z grafů, u serverových ramek model až moc ovlivňují latence a napětí. Při kapacitě 256GB a rychlosti 4800MHz zůstaly latenvce a napětí prázdný. Program je dosadil sám a vytvořil 2 dvojice - latence: 38, voltage: 1.1 a latence: 46, voltage: 1.2. Jedna kombinace byla za 90 000,- a druhá za 170 000. Blíže realitě (a to o dost) je ta dražší možnost.
...
- `Generation` (string, volitelné)
- `Is_kit` (bool, volitelné)
- `For_server` (bool, volitelné)
