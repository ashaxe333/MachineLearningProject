...
def run_ui():
    ...
    def on_submit():
        try:
            ...
            # Načtení volitelných (pokud prázdné, pošle None)
            g = int(combo_gen.get()) if combo_gen.get() else None
            b = entry_brand.get().strip() or "Unknown"
            l = float(entry_lat.get()) if entry_lat.get().strip() else None
            v = float(entry_volt.get()) if entry_volt.get().strip() else None
            k = 1 if var_kit.get() else 0

            results = predict_price(c, g, s, l, v, b, k)

            # Výpis výsledků do textového pole
    ...
    fields = [
        ...
        ("Brand (např. Corsair):", "entry_brand"),
    ...
    # Speciální prvky (Combo a Checkbutton)
    ...
    ttk.Checkbutton(main_frame, text="Jedná se o Kit (sadu)?", variable=var_kit).pack(pady=10)

    # Přiřazení proměnných pro snadnější přístup
    entry_cap = entries["entry_cap"]
    entry_speed = entries["entry_speed"]
    entry_brand = entries["entry_brand"]
